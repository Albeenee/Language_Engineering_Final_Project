...
const Rnn = props => {
    ...
    const sendDataToBackend = async (text, guesstype) => {
        ...
        const response = await axios.post('/gru', data);
    ...
    return (
      <Fragment>
        <h1>GRU</h1>
        <Form model='rnn' html='input' value={enteredText} inputChangeHandlerFunction={inputChangeHandler}/>
        {results && 
        <Suggestion model='rnn' results = {results} handlefunction={selectHandler}/>}
        {/* <KeystrokesCounter model='rnn' results={keystrokesCounter}/> */}
        <Button items={[{model:'rnn', link:"/", name:"Home"}]}/>
      </Fragment>
    );
}
